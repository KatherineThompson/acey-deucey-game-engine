...
function isValidTurn(gameState, diceRoll, proposedMoves) {
    ...
    function checkAceyDeuceyDice() {
        ...
        if (!_(proposedMoves).take(2).map("numberOfSpaces").sortBy().isEqual([1, 2])) {
            if (!areWinningMoves(_.take(proposedMoves, 2)) ||
                !_(proposedMoves).take(2).map("numberOfSpaces").sortBy().isEqual([1, 1])
            ) {
                return false;
            }
        }
    ...
    function checkDoublesDice() {
        ...
        if (proposedMoves.length < 4) {
            const numDoublesToAdd = proposedMoves.length - 2;
            const usedDoublesRoll = createSortedExtendedRoll(numDoublesToAdd, diceRoll[0]);
    ...
    function areDiceAndMovesEqual(diceRoll, moves) {
        function checkWinningMoves(diceRoll, moves) {
            const sortedMoves = _(moves).sortBy("numberOfSpaces")
                .forEach((move, index) => {
                    const proposedSpace = move.currentPosition + (move.numberOfSpaces * playerMultiplier);
                    if (proposedSpace === winningIndex) {
                        move.matchesDice = move.numberOfSpaces <= diceRoll[index];
                    } else {
                        move.matchesDice = move.numberOfSpaces === diceRoll[index];
                    }
                });
            return _.every(sortedMoves, "matchesDice");            
        }
        if (areWinningMoves(moves)) {
            if (diceRoll.length === 2 && diceRoll[0] !== diceRoll[1]) {
                // const numberOfSpacesMoved = _.map(moves, "numberOfSpaces");
                // const notMatchingDice = _.difference(diceRoll, numberOfSpacesMoved);
                // const notMatchingSpacesMoved = _.difference(numberOfSpacesMoved, diceRoll);
            ...
            checkWinningMoves(diceRoll, moves);
        }
